...
def byte_embajador() -> Response:
    ...
    try:
        knowledge_results: list[dict[str, Any]] = search_knowledge_base(
            db.session, user_message, threshold=0.6
        )

        if knowledge_results and knowledge_results[0]["relevance"] >= 0.6:
            response: str = knowledge_results[0]["answer"]
            logger.info("Respuesta de KB para Byte Embajador: %s", response)
        else:
            messages: list[dict[str, str]] = [
                {
                    "role": "system",
                    "content": (
                        "Eres Byte Embajador, un experto en Plubot y creación de chatbots. "
                        "Tu objetivo es ayudar a los usuarios a construir Plubots efectivos. "
                        "Responde de forma clara, amigable y educativa, usando un lenguaje "
                        "sencillo. Si no sabes la respuesta, sugiere buscar en la "
                        "documentación o contactar a soporte. Mantén las respuestas por "
                        "debajo de 150 palabras."
                    ),
                }
            ]
            ...
            cache_key: str = user_message.strip().lower()
            cached_response: Any | None = get_from_cache("byte_embajador", cache_key)

            if cached_response:
                logger.info(
                    "Respuesta de caché para Byte Embajador: %s", cached_response
                )
                response: str = cached_response
            else:
                response: str = call_grok(messages, max_tokens=200, temperature=0.7)
                logger.info("Respuesta de Byte Embajador (API): %s", response)
                store_in_cache("byte_embajador", cache_key, response)

        sentiment: str = analyze_sentiment(response)

        return jsonify({"message": response, "sentiment": sentiment})
    except Exception:
        logger.exception("Error in /api/byte-embajador")
        return jsonify({"error": "Error processing request"}), 500
...
def get_knowledge_by_category(category: str) -> Response:
    try:
        items = get_kb_by_category(db.session, category)
