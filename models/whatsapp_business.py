...
class WhatsAppBusiness(db.Model):
    ...
    # Fixed foreign key reference - must point to plubot table (singular)
...
class WhatsAppMessage(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    whatsapp_business_id = db.Column(db.Integer, db.ForeignKey("whatsapp_business.id"), nullable=False)
...
class WhatsAppWebhookEvent(db.Model):
    ...
    def __repr__(self) -> str:
        return f"<WhatsAppWebhookEvent {self.event_type} - {'Processed' if self.processed else 'Pending'}>"